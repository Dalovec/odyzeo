    ...
    <div class="mt-14 bg-slate-100 w-1/3 m-auto rounded-md shadow-xl">
        ...
        @foreach($tickets as $ticket)
            <div class="p-4 border-b-2 w-4/5 m-auto py-12">
                <a class="text-center text-slate-900 inline-block" href="/ticket?id={{$ticket->id}}">Ticket č. {{$ticket->id}}</a>@if($ticket->isOpen == 1)<a href="/close?id={{$ticket->id}}" class="inline-block float-right text-xs text-slate-50 bg-green-700 p-1 rounded text-center px-2 shadow">Open</a>@else<a class="inline-block float-right text-xs text-slate-50 bg-red-700 p-1 rounded text-center px-2 shadow">Closed</a>@endif
